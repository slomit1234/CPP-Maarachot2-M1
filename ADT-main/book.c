...
PElement copy_book(PElement elem) {
    pbook book = malloc(sizeof(struct BOOK_));
    if (!book) {
        return NULL;
    }
    pbook orig_book = (pbook)elem;
    book->name = strdup(orig_book->name);
    book->serial_number = orig_book->serial_number;
    return book;
}
